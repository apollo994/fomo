...
if [[ $# -ne 3 ]]; then
  echo "Usage: $0 <gff1.gff3> <gff2.gff3> <genome.fasta>" >&2
  exit 1
fi
...
# Auto-derive output path: same folder as GFF1, with .nonoverlapping inserted before the extension
gff1_base="${GFF1%.*}"             # strips last extension (e.g. .gff3 / .gff)
gff1_ext="${GFF1##*.}"             # keeps the original extension
OUT="${gff1_base}.nonoverlapping.${gff1_ext}"
echo "[INFO] Output GFF: $OUT"
...
command -v agat_sp_keep_longest_isoform.pl >/dev/null || { echo "[ERROR] agat_sp_keep_longest_isoform.pl not in PATH" >&2; exit 2; }
command -v agat_sp_extract_sequences.pl >/dev/null || { echo "[ERROR] agat_sp_extract_sequences.pl not in PATH" >&2; exit 2; }
...
# 8) Keep only the longest isoform per gene
longest_gff="${base}.longest.gff3"
agat_sp_keep_longest_isoform.pl -g "$OUT" -o "$longest_gff" >/dev/null
echo "[INFO] Longest isoform DONE: $longest_gff"
rm -f -- "${base}.longest_report.txt"

# 9) Extract merged exon sequences from the longest-isoform GFF
out_fa="${base}.longest.exons.fa"
agat_sp_extract_sequences.pl -g "$longest_gff" -f "$GENOME_FA" -t exon --merge -o "$out_fa" >/dev/null
echo "[INFO] Extract seq DONE: $out_fa"
