...
CMD_DIR="./lncRNA_map_commands"
mkdir -p "$CMD_DIR"

# Read lists safely (NUL-delimited)
mapfile -d '' refs < <(find data/ -type f -name '*.fna' -print0)
mapfile -d '' lncs < <(find data/ -type f -name '*lnc_RNA.exons.fa' -print0)
...
for ref in "${refs[@]}"; do
  ...
  cmdfile="$CMD_DIR/commands_lncmap_${ref_species}.sh"

  for lnc in "${lncs[@]}"; do
    lnc_species="$(basename "$(dirname "$lnc")")"
    bam="$outdir/${ref_species}_lncRNA_${lnc_species}.bam"
