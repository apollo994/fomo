...
CMD_DIR="./decoy_map_commands"
mkdir -p "$CMD_DIR"

# Read lists safely (NUL-delimited)
mapfile -d '' refs < <(find data/ -type f -name '*.fna' -print0)
mapfile -d '' lncs < <(find data/ -type f -name '*.aliasMatch.lnc_RNA.longest.nonoverlapping.decoy.exons.fa' -print0)
...
for ref in "${refs[@]}"; do
  ...
  cmdfile="$CMD_DIR/commands_decoymap_${ref_species}.sh"

  for lnc in "${lncs[@]}"; do
    lnc_species="$(basename "$(dirname "$lnc")")"
    bam="$outdir/${ref_species}_decoy_${lnc_species}.bam"
